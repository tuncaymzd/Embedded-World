...
faillist_parsed_data_failure_t faillist_parse_failure(char **faillist_ptr, int i) {
    ...
    failure.id_failure = faillist_parse_id_failure(faillist_ptr[i * 6 + 3]);
    failure.datetime = faillist_parse_date(faillist_ptr[i * 6 + 3 + 1]);
    failure.id_component = faillist_parse_id_component(faillist_ptr[i * 6 + 3 + 2]);
    failure.level_criticity = faillist_parse_level_criticity(faillist_ptr[i * 6 + 3 + 3]);
    failure.comment_failure_size = faillist_parse_comment_failure_size(faillist_ptr[i * 6 + 3 + 4]);
